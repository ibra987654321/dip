  ...
  <body>
    <div class="header">
      ...
      if(isset($_POST['submit'])){
        $search = $_POST['search'];
        $sql = "SELECT * FROM products WHERE `title` LIKE `%$search%` OR `opisanie` LIKE `%$search%` ";
        $query =mysqli_query($link, $sql);
        while($row = mysqli_fetch_assoc($query)){ echo '<h1>'.$row ['title'].'</h1><p>' .$row['opisanie']. '</p><br>';}
      } 
      ?>
      <div class="section__bottom">
        <div class="section__container"><img class="w-100" src="img/bunner/bunneriphone.jpg" alt=""/></div>
      </div>
    </div>
    <nav class="nav text-cenetr justify-content-center py-5"><a class="nav-link border btn btn-outline-primary rounded-pill border-primary text-bodyp-5" href="iphone.php">Iphone</a><a class="nav-link border btn btn-outline-primary rounded-pill border-primary text-bodyp-5" href="work.php">Samsung</a><a class="nav-link border btn btn-outline-primary rounded-pill border-primary text-bodyp-5" href="xiaomi.html">Xiaomi</a><a class="nav-link border btn btn-outline-primary rounded-pill border-primary text-bodyp-5" href="sony.html">Sony</a></nav>
    <div class="content container">
      ...
      <?php foreach($products as $product): ?>
        ...
        <div class="col-lg-3 pb-4"><img class="catalog" src="<?=$product ["photo"] ?>"/>
          <a href="post.php?post_id =<?=$product ["id"]?>" name="post_id"><?=$product ["title"] ?>
          <br> 
